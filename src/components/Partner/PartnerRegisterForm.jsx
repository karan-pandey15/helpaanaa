...
export default function PartnerRegisterForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      localStorage.setItem('helpaana_partner_registration', JSON.stringify(payload));
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8">
      <div className="text-center mb-2">
        <h2 className="text-2xl md:text-3xl font-bold text-[#0d5560]">Registration Form</h2>
        <p className="text-sm text-gray-500 mt-1">Complete all sections to register as a Helpaana partner</p>
      </div>
